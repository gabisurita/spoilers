...
pub fn impl_resource(ast: &syn::DeriveInput) -> quote::Tokens {
...
pub fn impl_collection_get(ast: &syn::DeriveInput) -> quote::Tokens {
    let config = MetaResourceConfig::new(ast.clone());
    let endpoint = config.endpoint().to_owned();
...
pub fn impl_collection_create(ast: &syn::DeriveInput) -> quote::Tokens {
    let config = MetaResourceConfig::new(ast.clone());
    let endpoint = config.endpoint().to_owned();
    ...
    quote! {
        ...
        fn #method_name(message: rocket_contrib::Json<serde_json::Value>, context: Context)
            ...
            let new: #form_name = match serde_json::from_value(message.0) {
                Ok(v) => v,
                Err(e) => {
                    return rocket_contrib::JsonValue(json!({"errors": ""}));
                }
            };
...
pub fn impl_pg_storage_backend(ast: &syn::DeriveInput) -> quote::Tokens {
    let config = MetaResourceConfig::new(ast.clone());
    let form_name = config.form_name();
    let model_name = config.model_name();
    let filter_name = config.filter_name();
...
pub fn impl_redshift_storage_backend(ast: &syn::DeriveInput) -> quote::Tokens {
    let config = MetaResourceConfig::new(ast.clone());
    let struct_name = config.struct_name();
    let form_name = config.form_name();
    let model_name = config.model_name();
